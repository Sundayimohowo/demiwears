...
function delete($table,$id)
  ...
  function update($table,$id,$data)
  {
   ...
   function getPublishedProducts(){
    global $conn;
    $sql="SELECT * FROM products WHERE publish=? ORDER BY id DESC LIMIT 1";

    $stmt= executeQuery($sql, ['publish' => 1]);
    $records=$stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    return $records;
}
function getProductsBycategory($new_id){
    global $conn;
    $sql="SELECT * FROM products WHERE publish=? AND category=? ORDER BY id DESC limit 9";

    $stmt= executeQuery($sql, ['publish' => 1, 'category'=> $new_id]);
    $records=$stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    return $records;
}

   function getPagePost($item_per_page,$offset){
    global $conn;
    $sql="SELECT n.*, u.username FROM news AS n JOIN users AS u ON n.user_id=u.id WHERE n.published=? ORDER BY id DESC LIMIT $item_per_page OFFSET $offset ";
